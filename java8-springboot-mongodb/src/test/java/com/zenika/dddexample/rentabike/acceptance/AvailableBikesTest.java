package com.zenika.dddexample.rentabike.acceptance;
...
class AvailableBikesTest {
    ...
    void aUserCanSeeAllAvailableBikesNearby() throws Exception {
        ...
        mockMvc.perform(get("/bikes?" + aroundMeQueryParams()))
                ...
                .andExpect(jsonPath("$[0].id").value(bikeNearby.getId().getValue()))
                .andExpect(jsonPath("$[0].position.lat").value(bikeNearby.getPosition().getLat()))
                .andExpect(jsonPath("$[0].position.lng").value(bikeNearby.getPosition().getLng()));
    }
    ...
    void aRentedBikeDoesNotAppearInAvailableBikesNearby() throws Exception {
        ...
        mockMvc.perform(post("/bikes/" + bikeNearby.getId().getValue() + "/scan"))
                .andExpect(status().isOk());
    ...
    void aReturnedBikeAppearsInAvailableBikesNearby() throws Exception {
        ...
        mockMvc.perform(post("/bikes/" + bike.getId().getValue() + "/scan"))
                .andExpect(status().isOk());

        mockMvc.perform(get("/bikes?" + aroundMeQueryParams()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].position.lat").value(bike.getPosition().getLat()))
                .andExpect(jsonPath("$[0].position.lng").value(bike.getPosition().getLng()));
    }
